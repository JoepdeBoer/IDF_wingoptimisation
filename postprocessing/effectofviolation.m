...
for i = 1:n
    ...
    CO2(i) = (exp(constant.R*LD^(-1)*C_T/V)/0.938-1)*(constant.W_aw+x(21)) * 3.16 / (5.255 * 10^4); % normalised to ref CO2 
end

computedLD  = ld *16.3
computedCO2 = (exp(constant.R*computedLD^(-1)*C_T/V)/0.938-1)*(constant.W_aw+x(21)) * 3.16 / (5.255 * 10^4); % normalised to ref CO2

actualLD = (xsol(19) - output.constrviolation) * 16.3;
actualCO2 = (exp(constant.R*actualLD^(-1)*C_T/V)/0.938-1)*(constant.W_aw+x(21)) * 3.16 / (5.255 * 10^4); % normalised to ref CO2

figure 
hold on 
plot(LDlist/xsol(19) , CO2, LineWidth= 2);
scatter(1, computedCO2, 50, 'filled');
scatter(actualLD/16.3/xsol(19),actualCO2, 50, 'filled')
